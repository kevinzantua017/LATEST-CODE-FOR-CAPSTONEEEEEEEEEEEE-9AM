...
# MUST be first so eventlet patches sockets/timers etc.
import eventlet
...
latest_status = {
    ...
    "action": "OFF",
    "scenario": "baseline",
    # kept for Archive table display
    "board_veh": "OFF",
    "board_ped_l": "OFF",
    "board_ped_r": "OFF",
}
...
def decide_scenario(now_ts: float, ped_count: int, veh_count: int,
    ...
    # baseline rules mirroring your worker logic
    action = "OFF"
    if tl_color == "red":
        action = "STOP"
    elif tl_color == "green":
...
def start_http_server(host="0.0.0.0", port=5000):
    ...
    # debug=False to keep eventlet performant
