...
This script initializes a set of USB cameras, captures frames from
the pedestrian, vehicle and traffic‑light feeds, and forwards JPEG
encodings to the MQTT broker.  It also subscribes to decision and
annotated frame topics coming from the cloud worker.  Decisions
published by the cloud contain the number of detected pedestrians
and vehicles, the inferred traffic‑light state, the distance to the
nearest approaching vehicle and the average vehicle speed.  Based
on these metrics and any active scenario, the Pi updates an LED
matrix display and forwards status messages to the dashboard.

The original implementation always showed "STOP" or "OFF" on the
LED board because the distance calculation treated any vehicle
whose bounding box extended below the virtual lane line as being
immediately at the crosswalk.  That resulted in a zero distance and
forced a STOP in most cases.  In this refactored version the
distance to the crosswalk is only computed for vehicles that are
above the lane; vehicles that have already crossed are ignored for
distance calculations.  The LED board now honours the inferred
scenario and displays descriptive messages for the rush‑hour,
pedestrian and emergency scenarios rather than just the raw
"STOP"/"GO"/"OFF" directives.

The file structure roughly follows the user's original submission.
Environment variables prefixed with ``SC_`` control camera
selection, frame rate, database logging cadence and MQTT settings.
